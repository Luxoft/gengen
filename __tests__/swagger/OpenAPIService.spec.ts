...
describe("OpenAPIService tests", () => {
    describe('ctor ', () => {
        test("old OpenApi version", () => {
            ...
            expect(() => new OpenAPIService(spec)).toThrow('Only OpenApi version 3 supported yet.');
        });
    ...
    describe('getEndpoints ', () => {
        test("not found", () => {
            ...
            const service = new OpenAPIService(spec);
    ...
    describe('getTagsByEndpoint ', () => {
        ...
        test.each(['get', 'post', 'put', 'delete'])('find', (operation) => {
            const spec = `{"openapi": "3.0.1", "paths": { "test": { "${operation}": { "tags": ["1", "2"] } } } }`;
            const service = new OpenAPIService(spec);
